...
int main(int ac, char** av) 
{
        ...
        char fileOrig[200];
	char fileProc[200];
        string window_name = "original | q or esc to quit";
        string window2_name = "processed | q or esc to quit";
        cout << "press space to save a picture. q or esc to quit" << endl;
        namedWindow(window_name, CV_WINDOW_KEEPRATIO); //resizable window;
	namedWindow(window2_name, CV_WINDOW_KEEPRATIO);

	VideoCapture cap(1);	//open the appointed camera. 0 >> default; 1 >> other
	if(!cap.isOpened())	//check if succeeded
		return -1;

	Mat original;//采集到的实时图像画面
	Mat gray;
	Mat processed;   //处理后的图像画面
//	namedWindow("video",1);
	for(;;)
	{
		cap >> original;
...
//{{{   退出程序或保存图像
            	...
            	switch (key) 
	    	{
        		...
        		case ' ': //Save an image
            			sprintf(fileOrig,"fileOrig%.3d.jpg",n);
				sprintf(fileProc,"fileProc%.3d.jpg",n++);
            			imwrite(fileOrig,original);
				imwrite(fileProc,processed);
            			cout << "Saved " << fileOrig << " and " << fileProc << endl;
            			break;
        		default:
            			break;
            	}
...
///		cvtColor(original, gray, CV_BGR2GRAY);	
			//转换图片的色彩空间，本语句将original的RGB通道转换为单通道，即灰度图片，输出为processed
	...
	///	GaussianBlur(gray, processed, Size(7,7), 1.5, 1.5);
...
//{{{   图像的二值化处理	
	//original = imread( fileOrig );
	cvtColor( original, gray, CV_BGR2GRAY); //将RGB图像转换为单通道的灰度图
	threshold(gray,processed,90,255,THRESH_BINARY);// 将灰度图进行二值化处理
//}}}

//{{{  处理前后的图像显示
		imshow(window2_name, processed);
